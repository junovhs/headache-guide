...
function checkRedFlags(answers: Map<string, Answer>): string[] {
	...
	if (suddenOnset?.value === "yes") {
		redFlags.push(
			"Sudden severe headache with maximum intensity reached within seconds to minutes",
		);
	}
...
export function getCategoryDescription(category: HeadacheCategory): string {
	const descriptions: Record<HeadacheCategory, string> = {
		...
		unknown:
			"The cause isn't clear from the available information. Try general relief measures.",
		red_flag:
			"Symptoms suggest a potentially serious condition requiring medical evaluation.",
	};
