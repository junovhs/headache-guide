...
export function renderResults(result: DiagnosticResult, onReset: () => void): void {
  ...
  if (immediateRemedies.length > 0) {
    remediesSection.appendChild(el('h3', { className: 'remedy-category', text: '⚡ Do Now' }));
  ...
  if (otherRemedies.length > 0) {
    remediesSection.appendChild(el('h3', { className: 'remedy-category', text: '📋 Also Consider' }));
...
function renderRemedy(remedy: Remedy): HTMLElement {
  ...
  header.appendChild(el('span', { className: 'remedy-time', text: `⏱️ ${remedy.timeToEffect}` }));
