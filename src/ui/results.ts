...
function createHeader(): HTMLElement {
	...
	header.appendChild(
		el("h1", { className: "results-title", text: "Assessment Complete" }),
	);
	return header;
}
...
function createDiagnosisCard(result: DiagnosticResult): HTMLElement {
	...
	card.appendChild(
		el("div", { className: "diagnosis-label", text: "Most Likely Cause" }),
	);
	card.appendChild(
		el("h2", {
	...
	for (const cause of causes) {
		...
		causeEl.appendChild(
			el("span", {
				...
				text: getCategoryLabel(cause.cause as any),
			}),
		);
		causeEl.appendChild(
			el("span", {
...
function createRemediesSection(result: DiagnosticResult): HTMLElement {
	...
	section.appendChild(
		el("h2", { className: "remedies-title", text: "Recommended Actions" }),
	);
