...
import type { Answer, AppState, DiagnosticSession } from "./types";
import {
	renderLoading,
	renderQuestion,
	renderResults,
...
function answerQuestion(answer: Answer): void {
	...
	const currentQuestion =
		applicableQuestions[state.session.currentQuestionIndex];
...
function checkForRedFlags(): boolean {
	...
	const suddenAnswer = state.session.answers.find(
		(a) => a.questionId === "red_flag_sudden",
	);
	if (suddenAnswer?.value === "yes") return true;

	const symptomsAnswer = state.session.answers.find(
		(a) => a.questionId === "red_flag_symptoms",
	);
