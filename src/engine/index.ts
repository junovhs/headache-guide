...
import { type CategoryScores, calculateScores, rankCauses } from "./scoring";

export { getCategoryDescription, getCategoryLabel } from "./labels";

import { getCategoryLabel } from "./labels";

function buildReasoning(
	scores: CategoryScores,
	primaryCategory: HeadacheCategory,
): string[] {
	const reasoning: string[] = [];
	const label = getCategoryLabel(primaryCategory);

	reasoning.push(
		`Based on your responses, this appears most likely to be a ${label} headache.`,
	);
